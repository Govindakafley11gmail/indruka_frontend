...
interface CardDetailsPageProps {
  ...
  formatPrice?: (price: number | string, country: string, isRegional?: boolean) => string;
}

// Regional visitors (IN/BD/BT) always see ₹, regardless of the tour's country.
// Non-regional visitors see Nu. for Bhutan tours, ₹ otherwise (unchanged original behavior).
const defaultFormat = (
  price: number | string,
  country: string,
...
export default function CardDetailsPage({
  ...
  const { isRegional, loading: regionLoading } = useVisitorRegion();
  ...
  // Compute the effective price for the currently selected tour (used by the modal)
  const selectedEffectivePrice = selectedTour
    ? isRegional && selectedTour.priceRegional
      ? selectedTour.priceRegional
      : selectedTour.price
    : 0;
  ...
  return (
    <div className="flex flex-col gap-[1.1rem]">
      ...
      {selectedTour && (
        <BookingModal
          pricePerPerson={selectedEffectivePrice ?? 0}
          guestsFieldId="number_of_travellers"
          tourName={selectedTour?.title ?? ""}
          price={String(selectedEffectivePrice ?? 0)}
          config={bhutanTripConfig(
            selectedEffectivePrice ?? 0,
            selectedTour.title,
